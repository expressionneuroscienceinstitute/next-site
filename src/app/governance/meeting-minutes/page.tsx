...
export const metadata = {
  title: 'Meeting Minutes | Expression Neuroscience Institute',
  description: 'Meeting minutes and records of the Expression Neuroscience Institute',
}

export default function MeetingMinutesPage() {
  const minutes = footerConfig.documentLinks.filter(doc => 
    doc.text.toLowerCase().includes('minutes')
  )

  return (
    <div className="container mx-auto px-4 py-12 max-w-4xl">
      <h1 className="text-3xl font-bold mb-8 text-center">Meeting Minutes</h1>
      
      {minutes.length > 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
          <ul className="space-y-4">
            {minutes.map((doc, index) => (
        ...
        <div className="text-center text-gray-500 dark:text-gray-400">
          <p>No meeting minutes are currently available.</p>
        </div>
      )}
