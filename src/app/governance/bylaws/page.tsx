...
export const metadata = {
  title: 'Bylaws | Expression Neuroscience Institute',
  description: 'Bylaws and foundational documents of the Expression Neuroscience Institute',
}

export default function BylawsPage() {
  const bylaws = footerConfig.documentLinks.filter(doc => 
    doc.text.toLowerCase().includes('bylaw')
  )

  return (
    <div className="container mx-auto px-4 py-12 max-w-4xl">
      <h1 className="text-3xl font-bold mb-8 text-center">Bylaws</h1>
      
      {bylaws.length > 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
          <ul className="space-y-4">
            {bylaws.map((doc, index) => (
        ...
        <div className="text-center text-gray-500 dark:text-gray-400">
          <p>No bylaws are currently available.</p>
        </div>
      )}
